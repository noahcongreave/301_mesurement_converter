        ...
        try:
            ...
            if value >= 10000:
                self.answer_error.config(
                    text="Number must be less than 10,000",
                    fg="#9C0000"
                )
            ...
            answer = self.conversions[choice](value)

            calculation = f"{choice}: {value} = {round(answer, 3)}"
...
class HistoryWindow:
    ...
    def export_data(self):
        ...
        self.export_filename_label.config(
            text=f"Export successful: {file_name}.txt",
            bg="#009900",
            fg="white"
        )
